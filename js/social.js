function initSocial() {
    const container = document.getElementById('social-grid');
    if (!container) return;

    // Funzione per creare una card (statica o cliccabile in base al link)
    function createStaticInstaCard(link, imageUrl, altText) {
        
        // Se il link è "-", creiamo una card semplice (solo immagine)
        if (link === "-") {
            return `
                <div class="block w-full max-w-[400px] h-[480px] bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden relative">
                    <div class="flex items-center justify-center h-full p-2 bg-slate-50"> 
                        <img 
                            src="${imageUrl}" 
                            alt="${altText}" 
                            class="w-full h-full object-contain" 
                            onerror="this.src='https://placehold.co/400x480?text=Immagine';"
                        >
                    </div>
                </div>
            `;
        }

        // Altrimenti, creiamo la card Instagram completa e cliccabile
        return `
            <a href="${link}" target="_blank" class="block w-full max-w-[400px] h-[480px] bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden relative group border border-slate-100">
                
                <div class="flex items-center justify-center h-full p-2 bg-slate-50"> 
                    <img 
                        src="${imageUrl}" 
                        alt="${altText}" 
                        class="w-full h-full object-contain transform group-hover:scale-105 transition-transform duration-700" 
                        onerror="this.src='https://placehold.co/400x480?text=Instagram+Post';"
                    >
                </div>
                
                <div class="absolute inset-0 bg-slate-900/0 group-hover:bg-slate-900/30 transition-colors duration-300"></div>
                
                <div class="absolute top-4 right-4 bg-white/90 backdrop-blur-sm p-2 rounded-full shadow-sm text-pink-600">
                    <i data-lucide="instagram" class="w-5 h-5"></i>
                </div>
                
                <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <span class="bg-white text-slate-800 font-bold py-3 px-6 rounded-full shadow-lg flex items-center gap-2 transform translate-y-4 group-hover:translate-y-0 transition-all">
                        Vai al post <i data-lucide="external-link" class="w-4 h-4 text-slate-400"></i>
                    </span>
                </div>
            </a>
        `;
    }

    // POST 1: Solo immagine
    const post1 = createStaticInstaCard(
        "https://www.instagram.com/p/DYwVZXIMljQ/?hl=it",
        "images/igieneor.wbep",
        "Post Instagram 1"
    );

    // POST 2: Solo immagine
    const post2 = createStaticInstaCard(
        "https://www.instagram.com/p/DY35FemIf5l/?hl=it",
        "images/sconto.wbep",
        "Post Instagram 2"
    );
    
    // POST 3: Post originale (cliccabile)
    const post3 = createStaticInstaCard(
        "https://www.instagram.com/p/DYg6t2FsQoU/?hl=it",
        "images/vichyprom.wbep",
        "Post Instagram 3"
    );
    
    // Inserisce i 3 post nella griglia
    container.innerHTML = post1 + post2 + post3;
    
    // Ricarica le icone
    if (typeof lucide !== 'undefined') {
        lucide.createIcons();
    }
}
